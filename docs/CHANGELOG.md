...
- **xterm.js Color Parity**: Aligned the ANSI color palette across the entire application (Monaco Editor, HTML Export) with the standard xterm.js (Tango) theme, ensuring a 1:1 visual match between live terminals and historical logs.
- **Enhanced Log Export Fidelity**: The HTML log export now correctly renders all 16 ANSI colors, including bright variants and background colors, using the professional xterm.js color specification.
- **Full-Spectrum ANSI Support**: Expanded the log viewer parsing engine to support missing color codes (Black, Bright Black, Bright White) and the entire range of background colors (40-47, 100-107).
